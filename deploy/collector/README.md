...
> 脚本采用「单时段采集」模式，不做常驻守护；跨时段靠调度器分别触发，也可以用仓库里的
> `ctp_data/service.sh` 常驻调度器反复拉起（见 `ctp_data/README.md` §5.1）。
>
> ⚠️ **容量前提不要照抄**：`dropped=0` 只说明内存缓冲没丢，**不等于覆盖率合格**。
> 2026-09-17 全市场实采显示：单个 SimNow 连接订阅约 1.7 万个合约时，白盘主力合约覆盖率
> 仅 0.0%~0.1%（证据见 `docs/迭代计划/迭代04-.../整改方案.md` 第 3 节）。生产部署必须先做
> 「订阅规模受控实验」（整改方案 P0-2）确定单连接可用上限，再决定分片/多连接。
...
| `--wait-open` | 在开盘前启动时**允许运行**（否则不在时段内会直接退出） |
| | ⚠️ 实际语义：它**不会真的等到开盘**，而是把"等待时长 + 本组时长 + grace"一起算进总时长，进程立即连接柜台并完成合约查询，然后一直采到收盘。对结果无害（慢查询反而在开盘前跑完），但别按"会等待"去推算启动时点 |
