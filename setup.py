...
CTP_DIR = ROOT / 'src' / 'bt_api_ctp' / 'ctp'
API_DIR = CTP_DIR / 'api' / '6.7.7'
WRAPPER = CTP_DIR / 'ctp_wrap.cpp'
EDITABLE_RUNTIME_DIRS = {
...
def _is_editable_runtime_artifact(file_name: str) -> bool:
    """Exclude in-place build runtime copies from source distributions."""
    ...
    return (
        len(parts) >= 4
        and parts[:3] == ('src', 'bt_api_ctp', 'ctp')
        and parts[3] in EDITABLE_RUNTIME_DIRS
    )


class SDist(sdist):
    """Make sdist independent of artifacts left by editable macOS builds."""

    def make_release_tree(self, base_dir: str, files: list[str]) -> None:
        retained_files = [
            file_name for file_name in files if not _is_editable_runtime_artifact(file_name)
        ]
        super().make_release_tree(base_dir, retained_files)
